...
extension AlbumDetailsViewController {
    func setUp() {
        ...
        goToAlbum.frame = CGRect(x: 20.0, y: self.view.frame.height - 140.0, width: self.view.frame.size.width - 40, height: 60.0)
        goToAlbum.backgroundColor = UIColor.init(red: 45/250, green: 140/250, blue: 123/250, alpha: 1)
        goToAlbum.layer.cornerRadius = 10.0
        goToAlbum.setTitle("Go To Album", for: .normal)
        ...
        if let albName = feedResult?.albumName {
            albumNameLabel.text  = "ALBUM NAME : " + albName
        }
